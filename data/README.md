...
A site run across separate campaigns splits into `<SITE>/<campaign>/`,
each a full site folder in its own right (`siteInfo.toml`, `scripts/`,
`raw/`, `utespac/`, `output/`, ...). `list_sites` descends one level
into any folder with no `siteInfo.*` of its own, so `data/MySite/2023`
and `data/MySite/2025A` are both discovered from `data/` and reported
as `MySite/2023` / `MySite/2025A`; `site_id` flattens the name for
product filenames (`MySite_2023_...`).

Two campaign-split patterns exist:

- **One folder per deployment year** (`MySite/2023`, `MySite/2025`):
  each year's `raw/` holds its own cards, independent of the others.
- **One folder per orientation/config span within a single deployment**
  (`MySite/2025A`, `MySite/2025B`): the spans share one underlying card
  pull. The shared `raw/` (and `binary/`, `scripts/`) live in a sibling
  folder that carries no `siteInfo.toml` of its own (`MySite/2025/`) --
  not a site, just a shelf for what the sibling spans' `scripts/`
  read from and write into each span's own `<campaign>/utespac/`. A
  span boundary exists because something changed mid-record (a boom
  reoriented, a planar fit needs recomputing separately either side of
  it) even though the raw instrument stream never stopped.
